 ...
 *
 * $Header: /home/jalembke/cvs/ConsoleTools/reboot/libannex/erpc_subr.c,v 1.1 2002/01/14 19:24:21 brylow Exp $
 *
 * This file created by RCS from:
 *
 * $Source: /home/jalembke/cvs/ConsoleTools/reboot/libannex/erpc_subr.c,v $
 *
 * Revision History:
 *
 * $Log: erpc_subr.c,v $
 * Revision 1.1  2002/01/14 19:24:21  brylow
 * - Source for terminal annex version of be-reset.
 *
 * Revision 8.0  1994/04/06  21:36:29  trinkle
 * Annex version 8.0
 *
 * Revision 1.13  92/11/13  14:36:05  carlson
...
#define RCSDATE $Date: 2002/01/14 19:24:21 $
#define RCSREV	$Revision: 1.1 $
#define RCSID   "$Header: /home/jalembke/cvs/ConsoleTools/reboot/libannex/erpc_subr.c,v 1.1 2002/01/14 19:24:21 brylow Exp $"
#ifndef lint
...
u_long get_unspec_long(), get_long();
...
int debug;		/* 1 iff -d switch seen by erpcd; 0 otherwise */

/*
...
#define	NPOLL 1
	...
	while (timeout > 0) {
		...
		for (;;) {
			...
			sdelay = delay;
			sdelay = sdelay << 10;
			nfound = select(32, &readfds, (long *)0, sdelay);
#else
			/* use real select */
			timeval.tv_sec = delay;
			timeval.tv_usec = 0;
			nfound = select(s + 1, &readfds, (int *)0, (int *)0,
					&timeval);
#endif /*EXOS*/
			/* error? */
